...
class Scraper:
    ...
    def get_scraped_courses(self, target: object, scraper):
        threads = []
        scraped_links = []
        for site in self.sites:
            t = threading.Thread(target=target, args=(site, scraper), daemon=True)
            t.start()
            threads.append(t)
            time.sleep(0.5)
        for t in threads:
...
class Udemy:
    ...
    def manual_login(self, email: str, password: str):
        ...
        session=s
        ss = cloudscraper.create_scraper(s)
        r = ss.post(
    ...
    def is_user_dumb(self) -> bool:
        """ this returns false if the user has removed one of these three entirely (categories/sites/languages) """
